...
function run() {
  ...
  sql-utils "$db" "select datetime(unix_timestamp, 'unixepoch') as time, heart_rate from heart_rate where unix_timestamp > $maxTimestamp" --csv | psql "$LIFELINE_CONNECTION_STRING" -c "COPY heart_rate FROM STDIN DELIMITER ',' CSV HEADER;"
